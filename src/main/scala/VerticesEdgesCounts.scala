...
object VerticesEdgesCounts {
  ...
  def verticesAndNodesCount(inputPath: String, spark: SparkSession, sc: SparkContext): String = {
    ...
    val graphRDD = sc.textFile("wasb://datasets@clouddeveloper.blob.core.windows.net/iterative-processing/Graph")
      var vertices = graphRDD.flatMap(line => line.split("\t"))
      val uni_lines = graphRDD.distinct()
      val uni_vers = vertices.distinct()
      val ver_pairs = uni_vers.map(ver => (ver, 1)).reduceByKey(_+_)
      val edge_pairs = uni_lines.map(edge => (edge, 1)).reduceByKey(_+_)
      val edges = 0L
      val vertices = 0L
